...
- 동기화(Syncronization)란 작업들 사이에 실행 시기를 맞추는 것을 뜻한다.
- 동기화 이슈는 여러 스레드가 동일한 데이터에 접근했을 때, 발생하는 오류를 뜻한다.
- 동기화 이슈 해결 방안들
  1. 임계영역(critical resource을 동시에 읽고 쓸 수 없도록 하는 부분)을 locking 하는 Mutual exclusion(상호 배제)을 이용한다.
   1) Mutex(binary semaphore): 임계 구역에 하나의 스레드만 들어가도록 허용한다.
   2) Semaphore: 임계 구역에 여러 스레드가 들어갈 수 있다. counter를 이용하여 동시에 리소스에 접근할 수 있는 허용 가능한 스레드 수를 제어한다.
